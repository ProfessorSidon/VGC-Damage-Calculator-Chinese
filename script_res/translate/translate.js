function translate(str) {
	...
	var word_idxs = [], idx_wordStart = -1, idx_last = -1, idx;
	...
	var names = [];
	for (var wordN = Math.min(5, word_idxs.length); wordN >= 1; wordN -= 1) {
		for (var i = 0; i <= word_idxs.length - wordN; i += 1) {
			var name = str.substring(word_idxs[i][0], word_idxs[i + wordN - 1][1]),
	...
	for (var i = 0; i < names.length; i += 1) {
		...
		for (translateFunc of TRYLIST) {
			var trans = translateFunc(name[0]);
			if (trans != name[0]) {
				return '' + trans + name[1];
			}
		}
...
function translate_move(name) {
	...
	var idx = name.indexOf('Hidden Power');
	if (idx != -1) {
		var type = name.substring(idx + 'Hidden Power'.length + 1);
...
function pokemonname_noforme(name) {
	...
	if (idx == 0) {
		var name0 = name.substring('Mega'.length + 1);
		return name0;
	}

	idx = name.indexOf('-');
	if (idx != -1) {
		var name0 = name.substring(0, idx);
		return name0;
	}
